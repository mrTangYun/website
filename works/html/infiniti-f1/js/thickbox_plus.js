...
function TB_show(caption, url) { //function called when the user clicks on a thickbox link
	try {
		...
		$("body").append("<div id='TB_load'><div id='TB_loadContent'></div>");
		$("#TB_overlay").show();
		...
		if(urlType == '.htm' || urlType == '.html'|| urlType == '.php'){//code to show html pages
			...
			typeString.push("注册/登录");
			typeString.push("选择赛区");
			typeString.push("恭喜！您已经成为英菲尼迪F1队员！");
			typeString.push("奖品兑换");
			typeString.push("奖品兑换");
			typeString.push("个人信息");
			typeString.push("北京F1观赛季VIP观赛证申请");
			typeString.push("观赛心愿投票");
			typeString.push("投票结果");
			...
			typeindex=params['type']*1;
			ajaxWindowTitle=typeString[typeindex];
			$("#TB_window").append("<div id='TopArea_AjaxWindow'><div id='Title_AjaxWindow'>"+ajaxWindowTitle+"</div><div id='TB_closeAjaxWindow'><a href='#' id='TB_closeWindowButton'><div></div></a></div><div class='clearFloat'></div></div><div id='TB_ajaxContent' style='width:"+ajaxContentW+"px;height:"+ajaxContentH+"px;'></div>");
...
function TB_position() {
    ...
    if ($.browser.msie) {
        if ($.browser.version == "6.0") {
			tt=$(window).scrollTop()+((h - TB_HEIGHT)/2);
		};
    }
	$("#TB_window").css({width:TB_WIDTH+"px",height:TB_HEIGHT+"px",
	left: ((w - TB_WIDTH)/2)+"px", top: tt+"px" });
