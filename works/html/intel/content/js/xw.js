		...
		success: function(media, node, player) {
				...
				for (var i=0, il=events.length; i<il; i++) {
					...
					media.addEventListener(events[i], function(e) {
						...
						if (e.type == 'play'){
							$("#popVideoLink").stop().animate({left:"-200px"});
						}
						else if (e.type == 'pause'){
							$("#popVideoLink").stop().animate({left:"0px"});
						}
					});
				}
			}
...
*/
	...
	var setRight = function(){
		...
		//$(".chooseBook").css("right",leftshowwidth+20);
	...
	var setLeftStage = function(){
		...
		$(".gototop").css({"bottom":"0px"});
		$(".gobottom").css({"bottom":"0px"});
	};

	//设置视频弹窗高度
	var setPopVideo = function(){
		var thisHeight = 596;
		//console.log($(window).width());
		$("#popVideo").css("top",(w_height-footer_height-header_height-thisHeight)/2).css("left",($(window).width()-980)/2).attr("data-x",($(window).width()-980)/2);
	...
	var pageInit = function(){
		...
		//绑定flash
		//swfobject.embedSWF("content/swf/860_573.swf", "flash", "860", "573", "9.0.0", "expressInstall.swf",null,{wmode:"transparent"});
	...
	var initStage = function(){
		...
		$(".stage").each(function(index, element) {
			...
			if (index == 0)
			{
				$this.before($stage_empty.clone().addClass(tclass).addClass("stage_empty-top"));
				$this.after($stage_empty.clone().addClass(bclass));
				$(".right_seKuai-top").css({"height":empty_height+442+"px"});
			} 
			else if (index == 4){
				$this.before($stage_empty.clone().addClass('tclass').addClass("stage_empty-bottom"));
				$this.after($stage_empty.clone().addClass('bclass').addClass("stage_empty-bottom"));
			}
			else{
	...
	//右侧色块
	function setRightSeKuai(){
		var w = ($(window).width()-1280)/2;
		$(".right_seKuai").css({width:w+"px"});
	}
	...
	function leftStageSelectChange(index){
		if(canScroll){
			...
			$(".right_seKuai").stop().animate({ marginTop: top*-1},1200);
	...
	function leftStageSelectBeforeChange(index,oldindex){
		...
		//player.pause();
		$("#popVideo").css({left:"9000px"});
		
		switch(index){
			case 1:
				$(".m_r_r_c").css("background-image","url(content/images/bx.jpg)");
				window.setTimeout(function(){
					/*场景2 特性1*/
					...
					$(".s4_6").stop().delay(100).animate({"top":"490px"},700);	


					$(".s2_3_1").stop().delay(200).animate({"top":"110px"},1500);	
					$(".s2_3_2").stop().delay(200).animate({"top":"210px"},1100);	
					$(".s2_3_3").stop().delay(200).animate({"top":"250px"},1000);	
					$(".stage_2_3_computer_1").stop().delay(900).fadeOut();
					$(".stage_2_3_computer_2").stop().delay(900).fadeIn();


				},200);
				break;
			case 2:
				$(".m_r_r_c").css("background-image","url(content/images/ck.jpg)");
				window.setTimeout(function(){
					
					$(".stage_3_videobg .js-item").each(function(i){
						$(this).stop().delay(200).animate({"top":$(this).attr("data-n-y")+"px","left":$(this).attr("data-n-x")+"px"},1500);
					});

				},200);
				break;
			case 3:
				$(".m_r_r_c").css("background-image","url(content/images/pt.jpg)");
				window.setTimeout(function(){
					$(".s4_1").stop().delay(200).animate({"top":"110px"},1500);	
					$(".s4_2").stop().delay(200).animate({"top":"210px"},1100);	
					$(".s4_3").stop().delay(200).animate({"top":"250px"},1000);	
		...
		switch(oldindex){
			...
			case 2:
				window.setTimeout(function(){
					...
					$(".stage_3_videobg .js-item").each(function(i){
						$(this).stop().delay(200).css({"top":$(this).attr("data-o-y")+"px","left":$(this).attr("data-o-x")+"px"});
					});
...
*/
	...
	//回到顶部按钮点击事件
	$(".gototop,.icon-gotoTop").click(function(e) {
	...
	function stage_2_play(){
		...
		$(".stage_3_mainbg_img-2").stop().delay(700).animate({"top":"-35px"});
		$(".stage_3_hand").delay(700).animate({"left":"50px"},800,function(){
		...
		$(".stage_4_right").stop().animate({"top":"268px"},1000,function(){
	...
	function stage_2_return_play(){
		...
		$(".stage_3_hand").stop().css({"left":"-150px"}).hide();	
		$(".stage_3_mainbg_img").stop().css({"top":"-50px"});
		$(".stage_3_mainbg_img-2").stop().css({"top":"-10px"});
	...
	function stage_3_play(){
		...
		$(".stage_3_mainbg_img-2").animate({"top":"-35px"},700);	
		$(".stage_3_hand").fadeIn(100);
		$(".stage_3_hand").delay(700).animate({"left":"50px"},800,function(){
			$(".hand_bowen").removeClass("hand_bowen_animal");
			setTimeout(function(){
    ...
    function changePeople(index){
    	$(".changeBook").hide();
    	bMouse = false;
    	//目标人物渐显--几秒后，笔记本进入--笔记本飞回去---人物淡出
    	$(".stage1_bg .peoples .item div:even").fadeOut(0);
    	$(".stage1_bg .peoples .item div:odd").hide();
    	$(".stage1_bg .peoples .item").eq(index).show().find("div").eq(0).fadeIn(1200);
    	$(".stage1_bg .peoples .item").eq(index).siblings().hide();
    	$(".people-other").show();
    	$(".people-5").hide();
    	$(".stage1_bg .peoples .el-bg").hide();
    	$(".stage1_right_copy .pop a").hide();
    	if (index <= 4){
    		var _t = parseInt($(".stage_1 .book").show().css("left"));
    			_t = (1400 - _t) / 1340 ;
    			_t = _t * 1500;
	    	$(".stage_1 .book").show().stop().animate({left:"1400px"},_t).animate({left:"60px"},1500,function(){
	    		bMouse = true;
	    		/*
	    		$(".stage1_bg .peoples .item").eq(index).find("div").eq(1).show().siblings().hide();*/
	    		$(".stage1_bg .peoples .item").eq(index).find("div").eq(1).fadeIn(1500).siblings().fadeOut(1200);
	    		$(".stage1_right_copy .copy h3").eq(index).fadeIn().siblings().hide();
	    		$(".stage1_right_copy .pop a[data-people="+index+"]").show();
	    		//changeBook(0);
	    	});
			
    		$(".stage1_right_copy .people-5 .photo .item-0").css("top","-500px");
    		$(".stage1_right_copy .people-5 .photo .item-1").css("top","-500px");
    		$(".stage1_right_copy .people-5 .photo .item-2").css("top","-500px");


    	}
    	else{
    		$(".stage_1 .book").stop().animate({"left":"1400px"},1200);
    		$(".people-other").hide();
    		$(".people-5").show();
    		$(".stage1_bg .peoples .el-bg").delay(500).fadeIn();
    		$(".stage1_right_copy .people-5 .photo .item-0").delay(500).animate({top:"0px"});
    		$(".stage1_right_copy .people-5 .photo .item-1").delay(200).animate({top:"79px"});
    		$(".stage1_right_copy .people-5 .photo .item-2").delay(100).animate({top:"114px"});
    	}
    }
    changePeople(0);
    $(".chooseBook .item").eq(0).addClass("red").siblings(".red").removeClass("red");
    changeBook(0);

    var timer_CP,i_CP;
   
    timer_CP=setInterval(function(){
    				i_CP = $(".chooseBook .item.red").index(".chooseBook .item");
    				i_CP = i_CP+1 > 5 ? 0 : i_CP+1 ;
					changePeople(i_CP);
					$(".chooseBook .item").eq(i_CP).addClass("red").siblings(".red").removeClass("red");
				},7000);


    //换人物按钮
    $(".chooseBook .item").click(function(){
    	location.hash=$(this).attr("href");
    	$(this).addClass("red").siblings(".red").removeClass("red");
    	var index = $(this).index();
    	//console.log(index);
    	changePeople(index);
    	clearInterval(timer_CP);    		
	    timer_CP=setInterval(function(){
	    				i_CP = $(".chooseBook .item.red").index(".chooseBook .item");
	    				i_CP = i_CP+1 > 5 ? 0 : i_CP+1 ;
						changePeople(i_CP);
    ...
    $(".changeBook a").mouseout(function(event){ 
    	return false;  
         event.stopPropagation();  
    });  


    $(".stage_1 .book .item").mouseenter(function(){
    	//if (bMouse){
		    var $changeBook = $(".changeBook");
		    $(this).find(".computer").append($changeBook);
		    $(".changeBook").fadeIn();
	    	clearInterval(timer_CP);  
    	//} 
    });
  $(".stage_1 .book .item").live('mouseleave',function(){
  		//if (bMouse){
  		$(".changeBook").fadeOut();	
	    timer_CP=setInterval(function(){
	    				i_CP = $(".chooseBook .item.red").index(".chooseBook .item");
    ...
    $(".stage_1 .book .item").mouseleave(function(){
    	console.log("mouseout");
    });
	*/
    /*
    $(".stage_1 .book .item").hover(function(){
    	console.log("mousein");
    	if (bMouse){
		    var $changeBook = $(".changeBook");
		    $(this).find(".computer").append($changeBook);
	...
	*/
    ...
    /*
    $(".stage_1 .book .item").click(function(){
	    $(".changeBook").fadeIn();
    	clearInterval(timer_CP);    		
	    timer_CP=setInterval(function(){
	    				i_CP = $(".chooseBook .item.red").index(".chooseBook .item");
	...
	*/
    //换笔记本按钮
    $(".changeBook .books a").click(function(){
    	var index = $(this).index();
    	changeBook(index);
    	/*
    	clearInterval(timer_CP);    		
	...
	function changeBook(index){
		...
		var $changeBook = $(".changeBook");
		$(".stage_1 .book>.item").eq(index).find(".computer").append($changeBook);
		$(".stage_1 .book>.item").eq(index).show().siblings().hide();
		$(".changeBook .books a").eq(index).addClass("red").siblings(".red").removeClass("red");
    	$(".changeBook .more span").eq(index).show().siblings("span").hide();
    	$(".changeBook .more a").eq(index).show().siblings("a").hide();
	}

	//鼠标滑过stage1的特性按钮
	$(".stage1_right_copy .pop a").hover(function(){
		var index = $(this).index();
		var text  = $(this).find("span").html();
		//console.log(text);
		if ($(this).find("span").length > 0)
		{
			$(".stage1_right_copy .pop .hoverText").show();
			//$(".stage1_right_copy .pop .hoverText .item").hide();
			$(".stage1_right_copy .pop .hoverText .item .text").html(text);
		}
	},function(){
		$(".stage1_right_copy .pop .hoverText").hide();
		//$(".stage1_right_copy .pop .hoverText .item").hide();
	});
	$(".stage1_right_copy .pop a").click(function(){
		return false;
	});

	/*场景2*/
	function changeFeature(index){
		$(".stage_2 .sliderContent .itemContent").each(function(i){

			$(".js-btn-changeFeature a").eq(index).addClass("red").siblings(".red").removeClass("red");
			if (index == i){
				if ($(".stage_2 .sliderContent .itemContent").eq(i).hasClass("red")){
					return;
				}
				$(".stage_2 .sliderContent .itemContent").eq(i).stop().addClass("red").animate({"left":"0px"},1200);
			}
			else{
				if ($(".stage_2 .sliderContent .itemContent").eq(i).hasClass("red")){
					$(".stage_2 .sliderContent .itemContent").eq(i).animate({"left":"-1280px"},1000,function(){
						$(".stage_2 .sliderContent .itemContent").eq(i).css({"left":"1280px"});
					}).removeClass("red");
				}
				else{
					$(".stage_2 .sliderContent .itemContent").eq(i).css({"left":"1280px"});
		...
		window.setTimeout(function(){
			switch(index){
				...
				case 1:
					...
					/*场景2 特性2*/
					$(".s2_1").stop().animate({"top":"51px"},1500);	
					$(".s2_2").stop().animate({"top":"145px"},1000);	
					$(".s2_3").stop().animate({"top":"185px"},800);
		...
		//$(".stage_2 .sliderContent .itemContent").eq(index).show().siblings(".itemContent").hide();
	}

    var timer_F,i_F;
    timer_F=setInterval(function(){
    				i_F = $(".js-btn-changeFeature a.red").index(".js-btn-changeFeature a");
    				i_F = i_F+1 > 3 ? 0 : i_F+1 ;
					changeFeature(i_F);
					
				},10000);

	$(".js-btn-changeFeature a").click(function(){
		var index= $(this).index();
		changeFeature(index);
	...
	function resetStage1(){
		...
		$(".s2_4").css({"top":"-96px"});	
		$(".s2_5").css({"top":"-96px"});
		$(".stage_2_right_p1").css({"top":"0px","left":"146px"});
		$(".stage_2_right_p2").css({"top":"26px","left":"190px"});
		$(".stage_2_right_p3").css({"top":"157px","left":"280px"});
		$(".stage_2_right").css({"margin-top":"-10px"});
		$(".stage_2_right_bg,.stage_2_right_xuxian").hide();
		...
		$(".s2_3_2").css({"top":"-96px"});	
		$(".s2_3_3").css({"top":"-96px"});
		$(".stage_2_3_computer_1").show();
		$(".stage_2_3_computer_2").hide();
	}

	$(".changeBook .more a").click(function(){
		leftStageSelectChange(3);
		return false;
	});
	...
	/*场景4*/
	$(".stage_4 .list .item").hover(function(){
		$(this).addClass("red");
	},function(){
		$(this).removeClass("red");
	});
	$(".stage_4 .tab a").click(function(){
		var index = $(this).index();
		showProList(index);
		return false;
	});
	function showProList(index){
		$(".stage_4 .tab a").eq(index).addClass("red").siblings(".red").removeClass("red");
		//$(".stage_4 .proList .list .page").eq(index).show().siblings().hide();
		$(".stage_4 .proList .list .js-Container").stop().animate({"margin-left":index*-966+"px"},1200);
	}

	//loadimg
	$(function(){
		var img = new Image();
		img.src = "content/images/ck.jpg";
		var img2 = new Image();
		img2.src = "content/images/bx.jpg";
		var img3 = new Image();
		img3.src = "content/images/pt.jpg";	
	});
...
/*
	$("#popVideo .closePop").click(function(){
		player.pause();
		$("#popVideo").hide();
		return false;
	});
...
//player.setSrc('mynewfile.mp4');
	...
	_title,分享标题（人人用）
	*/
	function share(_name,_url,_text,_title,_pic){
		 	_url=encodeURIComponent(_url);
			_title=encodeURIComponent(_title);
			_text=encodeURIComponent(_text);
			_pic=_pic.length==0?"":_pic;
		 
		 //if (_name!="tsina" && _name!="renren"){
			var u='http://www.jiathis.com/send/?webid='+_name+'&url='+_url+'&title='+_title+'&pic='+_pic+'&text='+_text+'&summary='+_text;
	...
	$(".rightTopSocial a").click(function(){

		share($(this).attr("data-name"),$(this).parent().attr("data-url"),$(this).parent().attr("data-text"),$(this).parent().attr("data-title"),'http://prcappzone.intel.com/ultrabook/ad/q3/content/images/Intel_UltrabookBasecamp_Weibo(440).jpg');
		return false;
	});
});
